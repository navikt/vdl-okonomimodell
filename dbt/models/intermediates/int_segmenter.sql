...
with
    ...
    pivot_table as (select * from {{ ref("int_hierarkier") }}),
    ...
    join_segment_with_hierarchy as (
        select
            ...
            rename_columns.* exclude (ar, kode, beskrivelse, segment_type)
        from segment_source
        left join
            rename_columns
    ...
    final as (
        ...
        union all
        select
            {{ dbt_utils.generate_surrogate_key(["null", "ar", "segment_type"]) }}
            as _uid,
            {{ dbt_utils.generate_surrogate_key(["null"]) }} as segment_id,
            {{ dbt_utils.generate_surrogate_key(["null", "ar"]) }} as segment_id_per_ar,
            segment_type,
            null as kode,
            ar,
            null as beskrivelse,
