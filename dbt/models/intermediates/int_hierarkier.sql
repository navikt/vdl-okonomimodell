...
with
    ...
    hierarchy_source as (select * from {{ ref("stg_oebs__segment_hierarkier") }}),

    segment_source as (select * from {{ ref("stg_oebs__segmenter") }}),
    ...
    max_level as (
        select *, max(niva) over (partition by id, hierarki) - niva as delta_niva
    ...
    default_level as (
        select distinct
            ...
            max(delta_niva) over (partition by id, hierarki)
            + 1
            || '_'
    ...
    levels as (
        ...
        group by all
        union all
        select ar, kode, beskrivelse, forelder, hierarki, segment_type
        from default_level
    ),
