...
with
    ...
    hierarchy_source as (select * from {{ ref("int_hierarkier__tertial_siste_navn") }}),
    ...
    recursive_hierarchy(
        ...
        select
            ...
            recursive_hierarchy.ar_tertial,
        ...
        join
            ...
            and recursive_hierarchy.ar_tertial = hierarchy_source.ar_tertial
            and coalesce(recursive_hierarchy.hierarki, 'default')
            = coalesce(hierarchy_source.hierarki, 'default')
    ),

    max_level as (
        select *, max(niva) over (partition by id) - niva as delta_niva
    ...
    default_level as (
        select distinct
            ...
            max(delta_niva) over (partition by id)
            + 1
            || '_'
            || lower(hierarki) as hierarki,
            segment_type
        from max_level
        where lower(hierarki) like 'intern%' and segment_type like 'OR_%'
    ),
    ...
    levels as (
        ...
        group by all
        union all
        select ar_tertial, kode, beskrivelse, forelder, hierarki, segment_type
        from default_level
    ),
    ...
    final as (select * from pivot_table)
